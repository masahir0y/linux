...
/*
 * The second handler for the internal DMA engine.
 */
irqreturn_t uniphier_sd_internal_dma_irq(int irq, void *devid)
{
					     ...
					     struct tmio_mmc_data *pdata)
{
	...
	/*
	 * Due to an unfortunate HW bug, Pro5 cannot use DMA for the RX.
	 * We can still use DMA for TX, but must use PIO for RX.
	 */
					   ...
					   struct mmc_data *data)
{
	...
	desc->callback = uniphier_sd_external_dma_callback;
				  ...
				  unsigned int clock)
{
	 ...
	 * bit10 is used to represent 1/1.  Newer versions of UniPhier variants
	 * use bit16 for 1/1024.
	 */
	if (divisor <= 1)
		tmp |= UNIPHIER_SD_CLK_CTL_DIV1;
...
static int uniphier_sd_probe(struct platform_device *pdev)
{
	...
	if (priv->caps & UNIPHIER_SD_CAP_EXTENDED_IP) {
		...
		ret = devm_request_irq(dev, irq, uniphier_sd_internal_dma_irq,
				       IRQF_SHARED, dev_name(dev), host);
		if (ret) {
			dev_warn(dev,
				 "failed to set-up internal DMA. falling back to PIO\n");
			host->dma_ops = NULL;
		}
	} else {
...
static const struct of_device_id uniphier_sd_match[] = {
	...
	{
		.compatible = "socionext,uniphier-sd-v3.1b",
		.data = (void *)UNIPHIER_SD_CAP_EXTENDED_IP,
	},
	{ /* sentinel */ }
};
